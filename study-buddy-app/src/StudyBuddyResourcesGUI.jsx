...
const MOCK_DATA = [
  { 
    ...
    type: "Video", 
    source: "Khan Academy", 
    desc: "Comprehensive breakdown of the 3 laws.",
    link: "https://www.khanacademy.org/science/physics/thermodynamics"
  },
  { 
    title: "Entropy Cheat Sheet", 
    type: "Notes", 
    source: "StudyBuddy AI", 
    desc: "Formula list for quick revision.",
    link: "https://hyperphysics.phy-astr.gsu.edu/hbase/Therm/entrop.html" 
  },
  { 
    title: "Heat Engines Quiz", 
    type: "Quiz", 
    source: "Internal Bank", 
    desc: "10 high-yield questions.",
    link: "https://www.google.com/search?q=heat+engines+practice+questions"
  }
];
              ...
              {!loading && recommendations.map((rec, idx) => {
                ...
                return (
                  <a 
                    ...
                    className={`block p-4 rounded-xl border hover:border-emerald-500 transition-all cursor-pointer group ${isStudyMode ? 'bg-navy-950 border-slate-700' : 'bg-white border-slate-100'}`}
                  >
  ...
  return (
    <div className={`flex h-screen font-sans transition-colors duration-500 ${isStudyMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
          ...
          {[
            ...
            <button
              ...
              className={`w-full flex items-center px-4 py-4 rounded-xl transition-all duration-200 group ${
                activeTab === item.id 
                ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-900/20 font-semibold' 
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className={`h-20 flex items-center justify-between px-8 border-b transition-colors ${isStudyMode ? 'bg-navy-950 border-slate-800' : 'bg-white border-slate-200'}`}>
          ...
          <button 
            ...
            className={`flex items-center px-5 py-2.5 rounded-full font-bold text-sm transition-all border ${
              isStudyMode 
              ? 'bg-navy-950 text-emerald-400 border-emerald-500/30 shadow-lg shadow-emerald-500/10' 
